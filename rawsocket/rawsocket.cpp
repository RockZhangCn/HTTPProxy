...
const int WIN_SIZE =  65535;

struct iphead{            //IP首部  
    uint8_t ip_hl:4, ip_version:4; //byte 低位，byte 高位
    uint8_t ip_tos;  
    uint16_t ip_len;  
    uint16_t ip_id;  
    uint16_t ip_off;  
...
int conn(int sendsockfd, /* int recsockfd,  */struct sockaddr_in seraddr){  // 三次握手  
    ...
    uint8_t packet[sizeof(struct iphead) + sizeof(struct tcphead)] = {0};  
    ...
    /*设置IP首部*/  
    ip->ip_hl = 5;   // 32bit * 5 = 20bytes.
    ip->ip_version = 4;  
    ip->ip_tos = 0;  
    ip->ip_len = htons(sizeof(struct iphead) + sizeof(struct tcphead));  //数据报的总长度。 
    //TODO.
    start_id = rand() & 0xFFFF;
    ip->ip_id = htons(start_id++); // rand();
    ...
    uint8_t buffer[1000] = {0}; //用于存储TCP伪首部和TCP报文，计算校验码  
    ...
    /*发送SYN报文段*/  
    int send_len = sizeof(struct iphead) + sizeof(struct tcphead);
    int send = sendto(sendsockfd, packet, send_len, 0, (struct sockaddr *)&seraddr, sizeof(seraddr));  
    if(send < 0){  
...
receive:
    ...
    printf("receive %d bytes:", n);  //将接受的IP数据报输出  
    for(int i=0; i<n; i++){  
        if(i % 16 == 0)  
            printf("\n");  
        printf("%.2x ", rec[i]);  
    ...
    for(int i=0; i<8; i++)  
        buffer[i] = rec[i + 12];  //获取源IP和目的IP  // 12 IP头部中源IP和目的IP的偏移量。
    ...
    uint32_t ack = ntohl(*((uint32_t *)(rec+ipheadlen+8))); //获取TCP首部的确认号  
    printf("ACK Number:%u\n", ack);  
...
int main() {  
    ...
    //connect
    conn(sendsockfd, seraddr); //模拟创建TCP连接  
    return 0;  
}  
